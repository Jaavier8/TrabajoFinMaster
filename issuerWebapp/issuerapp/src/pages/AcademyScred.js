...
export default function AcademyScred(props) {
  ...
  useEffect(() => {
    async function getData() {
      ...
      //Schemas
      const schIdsRes = await fetch(`${ACADEMY_ISSUER}/schemas/created`);
      if (schIdsRes.status === 200) {
        ...
        for (const schId of schIds) {
          const schRes = await fetch(`${ACADEMY_ISSUER}/schemas/${schId}`);
      ...
      //CredentialsDef
      const credIdsRes = await fetch(`${ACADEMY_ISSUER}/credential-definitions/created`);
      if (credIdsRes.status === 200) {
        ...
        for (const credId of credIds) {
          const credRes = await fetch(`${ACADEMY_ISSUER}/credential-definitions/${credId}`);
