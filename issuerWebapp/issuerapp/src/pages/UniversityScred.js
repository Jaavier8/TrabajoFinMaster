...
export default function UniversityScred(props) {
  ...
  useEffect(() => {
    async function getData() {
      ...
      //Schemas
      const schIdsRes = await fetch(`${UNIVERSITY_ISSUER}/schemas/created`);
      if (schIdsRes.status === 200) {
        ...
        for (const schId of schIds) {
          const schRes = await fetch(`${UNIVERSITY_ISSUER}/schemas/${schId}`);
